...
public:
    ...
    {
        if(isDeclared(fn.getName())) throw Error(ErrorCode::DuplicateDecl,"function \""+fn.getName()+"\" is duplicate declared!",pos);
        fn.setFid(next_fid++);
        functions.push_back(fn);
    }
    const Function & get(const std::string & name,const Position & pos) const
    {
        for(const auto & fn : functions)
        {
